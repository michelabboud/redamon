...
| **Attack Type** | Shows "CVE", "Brute Force", or technique-specific badge when the agent is executing an attack path. Unclassified paths appear with a grey badge |
...
| **Unclassified** (grey badge) | Fallback for techniques that don't match CVE or brute force (e.g., SQLi, XSS, SSRF). The agent uses available tools generically without a mandatory workflow |
